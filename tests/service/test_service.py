...
def test_invoke_custom_agent(test_client, mock_agent) -> None:
    ...
    mock_agent.ainvoke.return_value = [
        ("values", {"messages": [AIMessage(content=CUSTOM_ANSWER)]})
    ]
    ...
    with patch("service.service.get_agent", side_effect=agent_lookup):
        response = test_client.post(
            f"/{CUSTOM_AGENT}/invoke", json={"message": QUESTION}
        )
        assert response.status_code == 200
        ...
        assert output.type == "ai"
        assert (
            output.content == CUSTOM_ANSWER
        )  # Verify we got the custom agent's response
...
def test_invoke_model_param(test_client, mock_agent) -> None:
    ...
    response = test_client.post(
        "/invoke", json={"message": QUESTION, "model": CUSTOM_MODEL}
    )
    assert response.status_code == 200
    ...
    response = test_client.post(
        "/invoke", json={"message": QUESTION, "model": INVALID_MODEL}
    )
