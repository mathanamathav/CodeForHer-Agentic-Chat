...
async def test_ainvoke(agent_client):
    ...
    mock_request = Request("POST", "http://test/invoke")
    mock_response = Response(
        200, json={"type": "ai", "content": ANSWER}, request=mock_request
    )
...
async def test_acreate_feedback(agent_client):
    ...
    # Test successful response
    mock_response = Response(
        200, json={}, request=Request("POST", "http://test/feedback")
    )
    with patch("httpx.AsyncClient.post", return_value=mock_response) as mock_post:
...
def test_get_history(agent_client):
    ...
    # Mock successful response
    mock_response = Response(
        200, json=HISTORY, request=Request("POST", "http://test/history")
    )
    with patch("httpx.post", return_value=mock_response):
...
def test_info(agent_client):
    ...
    assert "Agent unknown-agent not found in available agents: custom-agent" in str(
        exc.value
    )
