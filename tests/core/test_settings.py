...
from schema.models import (AnthropicModelName, AzureOpenAIModelName,
                           OpenAIModelName)
...
def test_settings_no_api_keys():
    ...
    with patch.dict(os.environ, {}, clear=True):
        with pytest.raises(
            ValueError, match="At least one LLM API key must be provided"
        ):
...
def test_settings_azure_missing_deployment_names():
    with patch.dict(
        ...
        with pytest.raises(
            ValidationError, match="AZURE_OPENAI_DEPLOYMENT_MAP must be set"
        ):
